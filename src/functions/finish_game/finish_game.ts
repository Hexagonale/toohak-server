...
// const handler = async (body: z.infer<typeof schema>, request: Request): Promise<{ results: EndGameResult[] }> => {
...
//         const indexInRanking = body.current_ranking.findIndex((rankingPlayer) => rankingPlayer.token
// === player.token);
//         const currentPoints = body.current_ranking[indexInRanking].points;

//         const ranking = await rankingService.calculateEndGameRanking({
//             gameId: game.id,
...
//         eventsManager.notifyGameOver(ranking);
//     }
...
// // export const finish_game = onCall(functionErrorHandler(zodBodyValidator(schema, handler)));
// export const finish_game = functionWrapper(bodySchema, handler);
